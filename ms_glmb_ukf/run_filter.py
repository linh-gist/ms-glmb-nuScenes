...
from cppmsglmb import gibbs_multisensor_approx_cheap, gibbs_multisensor_approx_dprobsample, MCAdaptiveBirth, \
    bboxes_ioi_xyah_back2front_all, bboxes_ioi_xyah_back2front_all_v2, MSGLMB, meanShift, KMeans
...
def detection_aka_occlusion_model_v2(tindices_tt, model, q, ini_model_check, occ_model_on=True):
    ...
    P_3 = np.ones((4, X.shape[1]))
    P_3[0:3, :] = X[[0, 2, 4], :]
    temp = np.dot(model.cam_mat[q], P_3)
    test_point_Img_plane = temp[[0, 1], :] / temp[2, :]
    for j in range(len(pD_test)):
        f1 = test_point_Img_plane[0, j] <= 0
        f2 = test_point_Img_plane[0, j] >= model.imagesize[0]
        f3 = test_point_Img_plane[1, j] <= 0
        f4 = test_point_Img_plane[1, j] >= model.imagesize[1]
        if f1 or f2 or f3 or f4:
            pD_test[j] = np.spacing(0)  # if object is not in the image, then assign low pd.
...
class GLMB:  # delta GLMB
    ...
    def msjointpredictupdate(self, model, filter, meas, k):
        ...
        for s in range(model.N_sensors):
            ...
            for tabidx, tt in enumerate(glmb_predict_tt):
                for emm in tt.gatemeas[s]:
                    if meas[s].shape[0] == 6:
                        w_temp = tt.kalman_update_single(meas[s][:, emm], model.RR)
                    else:
        ...
        for pidx in range(0, len(self.glmb_update_w)):
            ...
            for s in range(model.N_sensors):
                ...
                if s == 6:
                    ...
                    avpd[:, s] = 0.5#detection_aka_occlusion_model_v2(tindices_tt, model, s, hypoth_num[pidx])
            avqd = np.log(1 - avpd)
            avpd = np.log(avpd)
            costc = []
        ...
        for tabidx in range(len(ttU_allkey)):
            ...
            qz_temp, tt = glmb_predict_tt[preidx].ukf_update(meas, meascomb[:6], model, k)
            lidar_indx = meascomb[6] - 1
            qz_temp_lidar = 0
            if lidar_indx >= 0:
                qz_temp_lidar = tt.kalman_update_single(meas[6][:, lidar_indx], model.RR, True)
            tt_update_msqz[tabidx] = (qz_temp + qz_temp_lidar)
    ...
    def generate_birth(self, meas_ru, meas_z, meas, model, k):
        for idx in range(meas[-1].shape[1]):
            m_lidar = meas[-1][:, idx]
            if meas_ru[-1][idx + 1] <= model.tau_ru:
                continue
            Ptemp = np.copy(model.P_birth)
            rbb = min(0.5, max(meas_ru[-1][idx], 0.2))
            m_birth = np.copy(model.m_birth)
            m_birth[[0, 2, 4], 0] = m_lidar[[0, 1, 2]]
            m_birth[[6, 7, 8], 0] = m_lidar[[3, 4, 5]]
            tt = Target(m_birth, Ptemp, rbb, self.id, None, model, k, self.use_feat)
            self.id += 1
            self.tt_birth.append(tt)
        # return
        if sum(meas_z[idx].shape[1] for idx in range(6)) < 1:
            return
        # (1) fix birth
        ...
        if self.adaptive_birth == 1:
            ...
            m_birth, r_birth, P_birth, sols_birth = self.meanshift_adaptive_birth(meas_ru[0:6], meas_z[0:6], model)
        ...
        if True:
            for idx, rbb in enumerate(r_birth):
                feat = np.ones((model.N_sensors, self.feat_dim))
                # for idx_s in range(model.N_sensors):
                #     m_idx = sols_birth[idx][idx_s]
                #     if m_idx:
                #         feat[idx_s, :] = meas[idx_s][5:, m_idx - 1]
                tt = Target(m_birth[idx], P_birth[idx], rbb, self.id, None, model, k, self.use_feat)
                self.id += 1
                self.tt_birth.append(tt)
            return
    ...
    def save_est_motformat(self, root, dataset, save_modes=False):
        ...
        for frame_id in range(len(self.est.X)):
            ...
            for eidx, tt in enumerate(targets.T):
                ...
                if save_modes:
                    line = [frame_id + 1, tt_id, x, y, z, dx, dy, dz, wx, wy, h, modes]
                    line_mot = [frame_id + 1, tt_id, -1, -1, -1, -1, -1, x, y, z, wx, wy, h, modes]
                else:
                    line = [frame_id + 1, tt_id, x, y, z, dx, dy, dz, wx, wy, h]
                    line_mot = [frame_id + 1, tt_id, -1, -1, -1, -1, -1, x, y, z, wx, wy, h]
                est_list.append(line)
                est_list_mot.append(line_mot)
        est = np.array(est_list)
        est_mot = np.array(est_list_mot)
        if est.shape[0] > 0:
            est[:, [8, 9, 10]] = np.exp(est[:, [8, 9, 10]])
            est_mot[:, [10, 11, 12]] = np.exp(est_mot[:, [10, 11, 12]])
        np.savetxt(os.path.join(root, 'EST_{}_WORLD_CENTROID_velocity.txt'.format(dataset)), est)

        class_names = {1: "pedestrian", 2: "pedestrian", 3: "car", 4: "truck", 5: "bus", 6: "trailer", 7: "motorcycle",
                       8: "bicycle"}
        # classname = class_names[int(dataset.split("_")[1])]
        # dataset = dataset.split("_")[0]
        np.savetxt(os.path.join(root, 'EST_{}_WORLD_CENTROID.txt'.format(dataset)), est_mot)
        print("==========>", "Saved", os.path.join(root, 'EST_{}_WORLD_CENTROID.txt'.format(dataset)))
        return est

    def run(self, model, dataset, meas, jsoncm, jsonlidar, class_name):
        ...
        #                8: "bicycle"}
        # class_names = {0: "pedestrian", 2: "car", 7: "truck", 5: "bus", 6: "trailer", 3: "motorcycle", 1: "bicycle"}
        # convert_class = {0: 0, 2: 1, 7: 2, 5: 3, 6: 4, 3: 5, 1: 6}
        for k, bboxes in enumerate(meas):
            model.m_birth = model.m_birth_class[class_name]
            model.RR = np.diag(model.wlhs_noise[class_name])
            model.cam_mat = [np.array(cm) for cm in jsoncm[str(k)]["prj_mats"]]
            model.cam_exts = [np.array(cm) for cm in jsoncm[str(k)]["cam_exts"]]
            ...
            lidar_boxes = np.array([box["center"] + box["wlh"] for box in jsonlidar[str(k)]["boxes"] if
                                    class_name == box["name"] and box["score"] > 0.4]).T
            if lidar_boxes.shape[0] == 0:
                lidar_boxes = np.empty((6, 0))
            else:
                lidar_boxes[3:6, :] = np.log(lidar_boxes[3:6, :] / 2)
            bboxes.append(lidar_boxes)
            model.N_sensors = 7
        ...
        return self.save_est_motformat("./results/nuscenes_cp05", dataset, save_modes=True)

    def runcpp(self, model, dataset, meas, jsoncm, jsonlidar, class_name, class_id, conf, adaptive_birth=3, use_feat=True):
        msglmbcpp = MSGLMB(model.cam_mat, "WILDTRACK", adaptive_birth, use_feat, class_id)
        st = time.time()  # get the start time
        # measurement in each sensor [0,1,2,3 (bbox: L T W H), 4 (conf), 5:end (re-id feature)]
        # start_off = int(len(meas) / 2)  # turn off all cameras for 30f
        tracking_names = ["bicycle", "bus", "car", "motorcycle", "pedestrian", "trailer", "truck"]
        class_names = {1: "pedestrian", 3: "car", 4: "truck", 5: "bus", 6: "trailer", 7: "motorcycle", 8: "bicycle"}
        convert_class = {1: 0, 3: 1, 4: 2, 5: 3, 6: 4, 7: 5, 8: 6}
        # [0, 1, 2, 3, 5, 7], ['person', 'bicycle', 'car', 'motorcycle', 'bus', 'truck']
        class_names = {0: "pedestrian", 2: "car", 7: "truck", 5: "bus", 6: "trailer", 3: "motorcycle", 1: "bicycle"}
        convert_class = {0: 0, 2: 1, 7: 2, 5: 3, 6: 4, 3: 5, 1: 6}
        for k, bboxes in enumerate(meas):
            m_birth = model.m_birth_class[class_name]
            RR = np.diag(model.wlhs_noise[class_name])
            PP = model.P_births[class_name]
            cms = [np.array(cm) for cm in jsoncm[str(k)]["prj_mats"]]
            cam_exts = [np.array(cm) for cm in jsoncm[str(k)]["cam_exts"]]
            msglmbcpp.setCamMats(cms, cam_exts, model.XMAX, model.YMAX, model.ZMAX, m_birth, RR, PP)
            # if start_off < k < start_off + 30:
            #     for s in range(model.N_sensors):
            #         bboxes[s] = np.empty((bboxes[s].shape[0], 0))
            scores = np.array([box["score"] for box in jsonlidar[str(k)]["boxes"] if class_name == box["name"]]).T
            scores = np.sort(scores)[::-1]
            if len(scores) > 2:
                threshold = scores[np.argmin(np.diff(scores))]
            else:
                threshold = 0.4
            threshold = float(conf) / 100
            lidar_boxes = np.array([box["center"] + box["wlh"] for box in jsonlidar[str(k)]["boxes"] if
                                    class_name == box["name"] and box["score"] > threshold]).T
            if lidar_boxes.shape[0] == 0:
                lidar_boxes = np.empty((6, 0))
            else:
                lidar_boxes[3:6, :] = np.log(lidar_boxes[3:6, :] / 2)
        ...
        et = time.time()  # get the end time
        print('Total Execution Time:', str(et - st), 'seconds', ', FPS', str(len(meas) / (et - st)))
        return self.save_est_motformat("./results/", dataset, save_modes=False)
    # END
