...
VectorXd detection_aka_occlusion_model_v2(vector<Target> tindices_tt, Model model, int q, int ini_model_check,
    ...
    MatrixXd P_3 = MatrixXd::Ones(4, X.cols());
    P_3(seq(0,3), all) = X(seq(0, 4, 2), all);
    MatrixXd temp = model.camera_mat[q] * P_3;
    MatrixXd test_point_Img_plane = temp(seq(0, 1), all).array() / temp(2, all).array();
    for (int j=0; j < pD_test.size(); j++){
        bool f1 = test_point_Img_plane(0, j) <= 0;
        bool f2 = test_point_Img_plane(0, j) >= model.imagesize[0];
        bool f3 = test_point_Img_plane(1, j) <= 0;
        bool f4 = test_point_Img_plane(1, j) >= model.imagesize[1];
        if (f1 || f2 || f3 || f4){
            pD_test[j] = std::nexttoward(0.0, 1.0L);  // if object is not in the image, then assign low pd.
        }
    }
...
private:
    ...
    void msjointpredictupdate(Model model, Filter filter, vector<MatrixXd> measZ, int k) {
        ...
        if (k == 0) {
            ...
            for (int s = 0; s < model.N_sensors; s++) {//for (MatrixXd bboxZ: measZ) {
                MatrixXd bboxZ = measZ[s];
            ...
            for (int tabidx = 0; tabidx < cpreds; tabidx++) {
                ...
                for (int emm = 0; emm < m(s); emm++) {
                    if (tt.mGateMeas[s](emm) >= 0) {
                        ...
                        double w_temp = 0;
                        if (measZ[s].rows() == 6){
                            w_temp = tt.kalman_update_single(measZ[s].col(emm), mModel.Rclass);
                        } else {
                            w_temp = tt.ukf_likelihood_per_sensor(measZ[s].col(emm), s);
                        }
                ...
                gatemeasidxs[s](tabidx, tt.mGateMeas[s]) = tt.mGateMeas[s];
            }
        }
        ...
        for (int pidx = 0; pidx < glmb_update_w.size(); pidx++) {
            ...
            for (int s = 0; s < model.N_sensors; s++) {
                ...
                if (s == 6){
                    //VectorXd tmpp = avpd.col(s);
                    avpd.col(s).array() = 0.9;
                } else {
                    avpd.col(s).array() = 0.5;//detection_aka_occlusion_model_v2(tindices_tt, model, s, hypoth_num[pidx]).array();
                }
            }
        ...
        for (int tt_idx = 0; tt_idx < tt_update_linidx.size(); tt_idx++) {
            if (umap.find(tt_update_linidx[tt_idx]) == umap.end()) {
                ...
                double qz_temp;
                Target tt;
                std::tie(qz_temp, tt) = glmb_predict_tt[preidx].ukf_update(measZ, meascomb(seq(0, 5)), k);
                int lidar_indx = meascomb[6] - 1;
                double qz_temp_lidar = 0;
                if (lidar_indx >= 0){
                    qz_temp_lidar = tt.kalman_update_single(measZ[6].col(lidar_indx), mModel.Rclass, true);
                }
                tt_update_msqz.push_back(qz_temp + qz_temp_lidar);
    ...
    tuple<vector<MatrixXd>, vector<double>, vector<vector<MatrixXd>>, vector<vector<int>>> meanshift_adaptive_birth(
        ...
        for (int idx = 0; idx < sols.size(); idx++) {
            ...
            if (solSum <= mModel.numDet || sols[idx][6]>0) {
                continue; // discarded solutions with few detections
            }
            VectorXd m_temp = mModel.m_birth.col(0);
            m_temp(0) = centroids[idx][0]; // x
    ...
    void generate_birth(vector<VectorXd> meas_ru, vector<MatrixXd> meas_z, vector<MatrixXd> meas, int k) {
        ...
        for (int idx=0; idx < meas_z[6].cols(); idx++){
            VectorXd m_lidar = meas_z[6].col(idx);
            if (meas_ru[6][idx + 1] <= mModel.taurU){
                continue;
            }
            MatrixXd m_birth = mModel.m_birth;
            vector<MatrixXd> Ptemp = mModel.P_birth;
            double rbb = min(0.9, max(meas_ru[6][idx], 0.5));
            m_birth(0, 0) = m_lidar(0);
            m_birth(2, 0) = m_lidar(1);
            m_birth(4, 0) = m_lidar(2);
            m_birth(6, 0) = m_lidar(3);
            m_birth(7, 0) = m_lidar(4);
            m_birth(8, 0) = m_lidar(5);
            Target tt(m_birth, Ptemp, rbb, mId, MatrixXd::Zero(10, 10), mModel, k, false);
            mId += 1;
        ...
        int num_meas = 0;
        for(int i=0; i<6; i++){
            num_meas += meas_z[i].cols();
        }
        if (num_meas < 1) {
            return;
        }//cout<<"0"<<endl;
        //meas_ru.pop_back();meas_z.pop_back();
        ...
        if (k == 0) {  // no information of birth (at the first time step), increase birth probability
            ...
            r_birth = v_ones;
        }//cout<<"r_birth.size() "<<r_birth.size()<<endl;
        for (int idx = 0; idx < r_birth.size(); idx++) {
            MatrixXd m_temp = m_birth[idx];
            vector<MatrixXd> Pb = P_b[idx];
            double r_b = min(0.25, max(r_birth[idx], 0.05));//r_birth[idx];
...
public:
    ...
    MSGLMB(vector<MatrixXd> camera_mat, string dataset = "CMC1", int adaptiveBirth = 1, bool useFeat = true, int startID=0) {
    ...
    void setCamMats(vector<MatrixXd> camera_mat, vector<MatrixXd> cam_exts, Vector2d x, Vector2d y, Vector2d z, VectorXd m_birth, MatrixXd RR, vector<MatrixXd> PP){
        mModel.camera_mat = camera_mat;
        mModel.cam_exts = cam_exts;
        mModel.XMAX = x;
        mModel.YMAX = y;
        mModel.ZMAX = z;
        mModel.N_sensors = 7;
        mModel.m_birth.col(0) = m_birth;
        mModel.Rclass = RR;
        mModel.P_birth = PP;
        //std::cout<<x.transpose()<<endl<<y.transpose()<<endl<<z.transpose()<<endl;
    }
