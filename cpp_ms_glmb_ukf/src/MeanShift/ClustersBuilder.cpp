...
std::vector<Cluster> ClustersBuilder::buildClusters(std::vector<int> sensors, std::vector<int> sensorsMeas) {
    std::vector<Cluster> clusters;

    // shifted points with distance minor or equal than clusterEps will go in the same cluster
    //int cout=0;
    for (int i = 0; i < shiftedPoints.size(); ++i) {//std::cout<<originalPoints[i][0]<<" "<<originalPoints[i][1]<<std::endl;
        Point shiftedPoint = shiftedPoints[i];
        auto it = clusters.begin();
        auto itEnd = clusters.end();
        while (it != itEnd) {
            float distance = it->getCentroid().euclideanDistance(shiftedPoint);
            if (distance <= clusterEps) {
                // the point belongs to a cluster already created
                it->addPoint(originalPoints[i], sensors[i], sensorsMeas[i], distance);//cout++;
                break;
            }
            ++it;
        }
        if (it == itEnd) {//cout++;
            ...
            cluster.addPoint(originalPoints[i], sensors[i], sensorsMeas[i], 1000);
            clusters.emplace_back(cluster);
        }
    }//std::cout<<"cout"<<cout;exit(0);
    return clusters;
}