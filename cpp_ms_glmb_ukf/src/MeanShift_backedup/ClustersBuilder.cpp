...
std::vector<Cluster> ClustersBuilder::buildClusters(std::vector<int> sensors) {
    ...
    for (int i = 0; i < shiftedPoints.size(); ++i) {
        ...
        while (it != itEnd) {
            ...
            if (distance <= clusterEps) {
                ...
                it->addPoint(originalPoints[i], sensors[i], distance);
                break;
            }
            ++it;
        }
        if (it == itEnd) {
            ...
            cluster.addPoint(originalPoints[i], sensors[i], 1000);
