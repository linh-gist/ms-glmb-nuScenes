...
int_t _ca_dense(
    ...
    for (int_t idx = 0; idx < n_free_rows; idx++) {
        int_t i = -1, j;
        uint_t k = 0;

        PRINTF("looking at free_i=%d\n", *pfree_i);
        j = find_path_dense(n, cost, free_rows[idx], y, v, pred);
        ASSERT(j >= 0);
        ASSERT(j < n);
        while (i != free_rows[idx]) {
            PRINTF("augment %d\n", j);
            PRINT_INDEX_ARRAY(pred, n);
            i = pred[j];
            PRINTF("y[%d]=%d -> %d\n", j, y[j], i);
 ...
 */
int lapjv_internal(const uint_t n, MatrixXd cost, VectorXi &x, VectorXi &y) {
    int ret;
    VectorXi free_rows(n);
    VectorXd v(n);
 ...
 */
int lapjv(MatrixXd cost, VectorXi &x, VectorXi &y, double cost_limit) {
    int cols = cost.cols();
    int rows = cost.rows();
    uint_t n = cols;
    if ((rows != cols) || cost_limit < std::numeric_limits<double>::infinity()) {
        n = rows + cols;
        MatrixXd cost_extended(n, n);
        if (cost_limit < std::numeric_limits<double>::infinity()) {
            cost_extended.array() = cost_limit / 2;
        } else {
            cost_extended.array() = cost.maxCoeff() + 1;
        }
        cost_extended.bottomRightCorner(cols, rows).array() = 0;
        cost_extended.topLeftCorner(rows, cols) = cost;
        cost = cost_extended;
    }
    x.resize(n);
    y.resize(n);
    ...
    if (ret != 0) {
        if (ret == -1) {
            ...
            std::cout << "Unknown error (lapjv_internal returned) . " << ret << std::endl;
        }
    }

    if (n != rows) {
        VectorXi x_temp = x(seq(0, rows - 1));
        x = (x_temp.array() >= cols).select(-1, x_temp);
    }

    if (n != cols) {
        VectorXi y_temp = y(seq(0, cols - 1));
        y = (y_temp.array() >= rows).select(-1, y_temp);
    }
