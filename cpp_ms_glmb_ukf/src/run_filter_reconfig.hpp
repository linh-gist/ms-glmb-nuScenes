...
VectorXd detection_aka_occlusion_model_v2(vector<Target> tindices_tt, Model model, int q, int ini_model_check,
    ...
    VectorXd ioa_max = 1 - ioa_temp.rowwise().maxCoeff().array();
...
private:
    ...
    void msjointpredictupdate(Model model, Filter filter, vector<MatrixXd> measZ, vector<bool> soff, int k) {
        // create birth tracks
        if (k == 0) {
        ...
        for (int s = 0; s < model.N_sensors; s++) {
            ...
            for (int tabidx = 0; tabidx < cpreds; tabidx++) {
                ...
                jointcostc[s] = (allcostc[s].colwise() + avps).array() - (NINF +  model.pdf_c);
        ...
        for (int pidx = 0; pidx < glmb_update_w.size(); pidx++) {
            ...
            for (int s = 0; s < model.N_sensors; s++) {
                ...
                avpd.col(s) = detection_aka_occlusion_model_v2(tindices_tt, model, s, hypoth_num[pidx])*NINF;
            }
    ...
    void generate_birth(vector<VectorXd> meas_ru, vector<MatrixXd> meas_z, vector<MatrixXd> meas, int k) {
        ...
        MatrixXd allcost = MatrixXd(mPrunedTargets.size(), sols_birth.size());
            ...
            for (int s = 0; s < mModel.N_sensors; s++) {
                ...
                if (m_idx) {
                    ...
                    for (int pidx = 0; pidx < mPrunedTargets.size(); pidx++) {
                        VectorXd subtract = mPrunedTargets[pidx].mFeat.row(s).array() - z_feat.array();
                        cdist_mat(pidx, s) = sqrt(subtract.array().square().sum());
                    }
                }
            }
            allcost.col(idxsol) = cdist_mat.rowwise().prod();
        }
        ...
        MatrixXd u_mapping = MatrixXd::Zero(u_label.size(), sols_birth.size());
        ...
        lapjv(u_allcost, x_assignments, y_assignments, 0.025);
        double allcost_sum = allcost.sum();
        ...
        for (int idx = 0; idx < sols_birth.size(); idx++) {
            ...
            if (y_assignments[idx] >= 0) {
                ...
                double r_b = std::min(mModel.r_birth[0], allcost(tt_idx, idx) / allcost_sum);
    ...
    std::tuple<MatrixXd, int, MatrixXi, vector<string>> extract_estimates(Model model) {
...
public:
    ...
    std::tuple<MatrixXd, int, MatrixXi, vector<string>> run_msglmb_ukf(vector<MatrixXd> measZ, vector<bool> soff, int kt) {

        msjointpredictupdate(mModel, filter, measZ, soff, kt);
